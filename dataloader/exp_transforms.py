...
class SpatialAug(object):
    ...
    def __call__(self, inputs, target, intr):
        ...
        # object index
        target_0[:,:,-1] = F.grid_sample(torch.Tensor(target[:,:,-1:]).permute(2,0,1)[np.newaxis],    vgrid[np.newaxis], mode='nearest', align_corners=True)[0].permute(1,2,0)[:,:,-1]
        # sparse depth
        if target[:,:,3].max()==np.inf:
            target_0[:,:,3] = F.grid_sample(torch.Tensor(target[:,:,3:4]).permute(2,0,1)[np.newaxis],    vgrid[np.newaxis], mode='nearest', align_corners=True)[0].permute(1,2,0)[:,:,0]
